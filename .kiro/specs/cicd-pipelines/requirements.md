...
3. THE CICD_Stacks SHALL use a dedicated `cdk.json` in a separate
   subdirectory or a dedicated `--app` argument pointing to a different
   entry-point file, so that CICD_Stacks are never co-synthesized with
   App_Stacks under the project-root `cdk.json`
4. THE CICD_Stacks SHALL not import any TypeScript module that is exported
   by App_Stacks source files (`lib/` constructs used by `bin/app.ts`),
...
### Requirement 2: GitHub Actions のみのパイプライン
...
1. WHEN a push event occurs on a branch specified in the workflow YAML
   trigger configuration, THE GitHub_Actions_Pipeline SHALL trigger
   a workflow run
2. THE GitHub_Actions_Pipeline SHALL execute
   `cdk deploy --all -c prefix=<value>` to deploy App_Stacks against
   the development environment, where `<value>` is defined as a workflow
...
6. THE GitHub_Actions_Pipeline workflow SHALL define a `timeout-minutes`
   value of no more than 30 minutes per job to prevent runaway executions

### Requirement 3: CodePipeline V2 のみのパイプライン
...
1. THE CodePipeline_V2_Pipeline SHALL be created as a V2 type pipeline
   using CDK constructs within CICD_Stacks
2. WHEN a code change is pushed to the configured branch in the GitHub
   repository, THE CodePipeline_V2_Pipeline SHALL trigger a pipeline
   execution via a CodeStar Connections source action
3. THE CodePipeline_V2_Pipeline SHALL include a source stage
   that retrieves code from the GitHub repository using
   AWS CodeStar Connections
4. THE CodePipeline_V2_Pipeline SHALL include a build/deploy stage
   that installs dependencies, runs `cdk synth`, and then executes
   `cdk deploy --all` with the appropriate prefix context
5. IF the deploy stage fails,
   THEN THE CodePipeline_V2_Pipeline SHALL mark the pipeline execution
   as failed

### Requirement 4: GitHub Actions + CodePipeline V2 ハイブリッドパイプライン
...
1. WHEN a push event occurs on a branch specified in the workflow YAML
   trigger configuration, THE Hybrid_Pipeline SHALL trigger
   a GitHub Actions workflow
2. THE GitHub Actions workflow in the Hybrid_Pipeline SHALL package
...
3. THE GitHub Actions workflow in the Hybrid_Pipeline SHALL invoke
   `codepipeline:StartPipelineExecution` API to trigger CodePipeline V2,
   passing Pipeline_Arguments (branch name and commit SHA)
   as pipeline variables
4. THE CodePipeline V2 in the Hybrid_Pipeline SHALL use an S3 source
...
5. THE CodePipeline V2 build stage SHALL receive the pipeline variables
   (branch name, commit SHA) as environment variables in CodeBuild,
   and SHALL execute `cdk deploy --all` with the appropriate prefix context
6. THE CodePipeline V2 in the Hybrid_Pipeline SHALL be a V2 type pipeline
   created with CDK constructs within CICD_Stacks
7. THE Source_Bucket SHALL be created as part of CICD_Stacks
8. IF the S3 upload or StartPipelineExecution call fails,
   THEN THE GitHub Actions workflow SHALL fail the workflow run
   with a non-zero exit code
9. IF the deploy stage in CodePipeline V2 fails,
   THEN THE pipeline execution SHALL be marked as failed

### Requirement 5: CodePipeline リソースの CDK による管理
...
1. THE CICD_Stacks SHALL define CodePipeline V2 パイプライン、
   ...
   デプロイアクション、および Artifact 用 S3 バケットを
   CDK コンストラクトで定義する
...
2. THE CICD_Stacks stack names SHALL follow the pattern
   `${prefix}-cicd-${pipeline-type}` where pipeline-type is one of
   `codepipeline` or `hybrid`
   (e.g., `dev-cicd-codepipeline`, `dev-cicd-hybrid`);
...
3. WHEN physical resource names are required
   ...
   pipeline-type suffix to ensure uniqueness across all three
   CI/CD patterns deployed simultaneously
   (e.g., `${prefix}-cicd-hybrid-source` for the Source Bucket)
